...
public class SpringAPI implements JbillingAPI {
	...
	public Integer[] getLastInvoicesByItemType(Integer userId,
			Integer itemTypeId, Integer number) throws JbillingAPIException {
		try {
			return session
					.getLastInvoicesByItemType(userId, itemTypeId, number);
		} catch (Exception e) {
			throw new JbillingAPIException(e);
		}
	}
	...
	public InvoiceWS getLatestInvoiceByItemType(Integer userId,
			Integer itemTypeId) throws JbillingAPIException {
		try {
	...
	public ItemDTOEx getItem(Integer itemId, Integer userId,
		...
		try {
			return session.getItem(itemId, userId,
	...
	/*
	 * @see
	 * com.sapienter.jbilling.server.util.api.JbillingAPI#processPayment(com
	 * .sapienter.jbilling.server.payment.PaymentWS)
	 */
	@Override
		...
		try {
			...
			if (fields != null) {
				...
				for (int i = 0; i < pricingFields.length; i++) {
					pricingFields[i] = PricingField
							.setPricingFieldsValue(fields[i]);
				}
			}
			return session
					.validateMultiPurchase(userId, itemIds, pricingFields);
		} catch (Exception e) {
			throw new JbillingAPIException(e);
		}
	}
	...
	public void setAutoPaymentType(Integer userId, Integer autoPaymentType,
			boolean use) throws JbillingAPIException {
		try {
	...
	public void deleteItemType(Integer itemTypeId) throws JbillingAPIException {
		try {
			session.deleteItemType(itemTypeId);
		} catch (Exception e) {
			throw new JbillingAPIException(e);
		}
	}
